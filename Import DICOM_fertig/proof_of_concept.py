...
parser = ConfigParser()
parser.read('dev2.ini')

# get the sections from the ini
sections = parser.sections()

# STORE RELATION-IDS WITH THEIR PROVENANCES
relations_with_provenances = {}
...
allRelationTypes_with_static = allRelationTypes + ['hasPatient', 'hasFile', 'belongsToPatient', 'hasSex', 'hasBirthDate', 'hasDate', 'hasInstanceCreationDate', 'hasAcquisitionDate']
...
class Relation:
    def __init__(self, _start, _end, _type, _prov):
        ...
        self.relationUID = str(_start) + str(_type) + str(_end) + str(_prov)

# ---------------------------------------------------------------------------------------------------------------------

def print_static(static_nodes, static_type):
    # create new dir if necessary
    export_nodes_to = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/Import/nodes'
    ...
    with open(f"{static_type}.csv", 'w') as csvfile:
        ...
        for n in static_nodes:
            csv_node_writer.writerow([n.ID, n.Identifier, n.Identifier])
...
# find root directory
rootdir = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/PRST-20080922'
...
for elem in paths:
    ...
    for file in listOfFiles:
        ...
        if file.endswith('.dcm'):
            ...
            for element in sections:
                ...
                if parser[element]['Date'] != '':
                    ...
                    if date_time_id not in dates:
                        ...
                        # create dic
                        date_time_dic = {'ID': date_time_id, 'Date': date_from_dicom, 'Time': time_from_dicom, 'nodeUID': date_time_id}
                        # create date node
                ...
                for i in range(len(relationsTo)):
                    try:
                        # CREATE RELATION ID TO STORE IN DICTIONARY AND LATER COMBINE PROVENENCES TO REDUCE RELATIONS
                        relationID_without_prov = str(uid) + str(filewise_uid_dic[relationsTo[i]]) + str(relationTypes[i])
                        # ONLY CREATE RELATION IF IT'S NOT ALREADY EXISTING
                        if relationID_without_prov not in relations_with_provenances:
                            ...
                            relations_with_provenances[relationID_without_prov] = {provenance}
                        # IF RELATIONSHIP IS ALREADY EXISTING ONLY ADD THE PROVENANCE TO THE LIST
                        else:
                            relations_with_provenances[relationID_without_prov].add(provenance)
                    except KeyError:
                        continue
                    # THE AFOREMENTIONED CONCEPT IS KEPT AND USED FOR OTHER RELATIONS AS WELL
                # create relations from IOD module to date and time (if necessary)
                if date_boolean == 1:
                    relationID_without_prov = str(uid) + str(date_time_id) + str(date_relation_type)
                    if relationID_without_prov not in relations_with_provenances:
                        ...
                        relations_with_provenances[relationID_without_prov].add(provenance)
                if sex_boolean == 0:
                    relationID_without_prov = str(ds['PatientID'].value) + str(ds['PatientSex'].value) + 'hasSex'
                    if relationID_without_prov not in relations_with_provenances:
            ...
            # ALSO CHECK FOR POSSIBLE DOUBLES (EXCEPT PROVENANCES)
            relationID_without_prov = str(file) + str(ds['PatientID'].value) + 'belongsToPatient'
            if relationID_without_prov not in relations_with_provenances:
            ...
            relationID_without_prov = str('PRST-20080922') + str(file) + 'hasFile'
            if relationID_without_prov not in relations_with_provenances:
                source_file_rel = Relation('PRST-20080922', file, 'hasFile', provenance)
                source_file_relations.append(source_file_rel)
                relations_with_provenances[relationID_without_prov] = {provenance}
            else:
                relations_with_provenances[relationID_without_prov].add(provenance)
            # relation from source to patient
            relationID_without_prov = str('PRST-20080922') + str(ds['PatientID'].value) + 'hasPatient'
            if relationID_without_prov not in relations_with_provenances:
                source_patient_rel = Relation('PRST-20080922', ds['PatientID'].value, 'hasPatient', provenance)
...
def print_edges(edges_to_print, edgeType):
    ...
    # create new dir if necessary
    export_nodes_to = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/Import/relations'
    ...
    with open(f"{edgeType}.csv", 'w') as csvfile:
        ...
        for e in edges_to_print:
            csv_edge_writer.writerow([e.start_id, e.end_id, e.rel_type, e.provenance, e.relationUID])


# print header for nodes
def print_variable_node_headers(var_node):

    temp_dir = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/Import/CSVHeaderFiles/nodes'
...
def print_static_node_headers():
    ...
    with open('Source-header.csv', 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':ID', 'Source', 'nodeUID'])

    # file node
    with open('File-header.csv', 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':ID', 'File', 'nodeUID'])

    # sex node
    with open('Sex-header.csv', 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':ID', 'Sex', 'nodeUID'])

    # sex node
    with open('Usage-header.csv', 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':ID', 'Usage', 'nodeUID'])

    # Date And Time node
    with open('DateAndTime-header.csv', 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':ID', 'Date', 'Time', 'nodeUID'])


def print_variable_edge_headers(edge_type):
    temp_dir = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/Import/CSVHeaderFiles/relations'
    ...
    with open(f"{edge_type}-header.csv", 'w') as csvfile:
        csv_header_writer = csv.writer(csvfile)
        csv_header_writer.writerow([':START_ID', ':END_ID', 'type', 'provenance', 'relationUID'])


def print_additional_provenances():
    temp_dir = '/Users/tobias/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni/Masterarbeit/proof of concept data/Import'
    ...
    with open("additional_provenances.csv", 'w') as csvfile:
        provenances_writer = csv.writer(csvfile)
        for key in relations_with_provenances:
            provenances_writer.writerow([key, relations_with_provenances[key]])
