...
def create_sents():
    ...
    #Q12 MATCH (n1)-[]-(p1:General_Study)-[]-(p2:General_Series)-[]-(n2) RETURN p1.nodeUID as pathNode1, p2.nodeUID as pathNode2, n1.nodeUID as NeighbourOfp1, n2.nodeUID as NeighbourOfp2 ORDER BY p1.nodeUID, p2.nodeUID, n1.nodeUID, n2.nodeUID
...
def create_sents_from_import():
    ...
    q_path_import = []
    row_counter = 0
    with open('exportQ11adamicAdar.csv', newline='') as csvfile:
...
def nodes2features(s, i, d):
    ...
    # use closeness
    ft['prefAttach'] = graph.run(f"MATCH (n1 {temp1}nodeUID: '{node}'{temp2}) MATCH(n2 {temp1}nodeUID: '{s[i][1]}'{temp2}) RETURN gds.alpha.linkprediction.preferentialAttachment(n1,n2)").data()[0]['gds.alpha.linkprediction.preferentialAttachment(n1,n2)']
